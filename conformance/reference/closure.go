...
func facts(h []Observation)[]Fact{m:=map[string]Fact{};for i,o:=range h{for _,x:=range o.Available{k:=fmt.Sprintf("A|%s|%d",x,i);m[k]=Fact{"AVAILABLE",[]string{x},[]int{i}}};if o.LastAction!=nil{x:=o.LastAction.(string);k:=fmt.Sprintf("X|%s|%d",x,i);m[k]=Fact{"ACTION",[]string{x},[]int{i}}};if i>0&&o.LastAction!=nil{x:=o.LastAction.(string);p:=map[string]bool{};c:=map[string]bool{};for _,t:=range h[i-1].Available{p[t]=true};for _,t:=range o.Available{c[t]=true};all:=map[string]bool{};for t:=range p{all[t]=true};for t:=range c{all[t]=true};ks:=make([]string,0,len(all));for t:=range all{ks=append(ks,t)};sort.Strings(ks);for _,t:=range ks{if !p[t]&&c[t]{k:=fmt.Sprintf("I|%s|%s|%d",x,t,i-1);m[k]=Fact{"INTERVENES",[]string{x,t},[]int{i-1}};k=fmt.Sprintf("E|%s|%s|%d",x,t,i-1);m[k]=Fact{"OBSERVED_EFFECT",[]string{x,"ENABLES("+t+")"},[]int{i-1}}}else if !p[t]&&!c[t]{k:=fmt.Sprintf("N|%s|%s|%d",x,t,i-1);m[k]=Fact{"NONENABLES",[]string{x,t},[]int{i-1}}}};if mr,ok:=o.LastResult.(map[string]any);ok{if s,_:=mr["status"].(string);s=="BLOCKED"{k:=fmt.Sprintf("B|%s|%d",x,i);m[k]=Fact{"BLOCKED",[]string{x},[]int{i}}}}}};out:=make([]Fact,0,len(m));for _,f:=range m{out=append(out,f)};sort.Slice(out,func(i,j int)bool{return string(canon(out[i]))<string(canon(out[j]))});return out}
