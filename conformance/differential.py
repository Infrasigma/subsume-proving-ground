...
def legacy():
    ...
    for c in cases:
        prod=run([sys.executable,str(LEGACY_PROD)],c); ref=run(['go','run',str(LEGACY_REF)],c); compare(prod,ref,c['name'])
        exp=c['expected']
        ...
        clean=run([sys.executable,str(LEGACY_PROD)],c); assert clean==prod,c['name']+' environment leakage'
        print('PASS legacy_regression',c['name'])

def closure():
    cases=json.loads(CLOSURE_FIX.read_text())
    for c in cases:
        prod=run([sys.executable,str(CLOSURE_PROD)],c); ref=run(['go','run',str(CLOSURE_REF)],c); compare(prod,ref,c['name'])
