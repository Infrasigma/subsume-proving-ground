...
def word(ns,seed,counter,condition="",purpose=""): return int.from_bytes(hashlib.sha256(stream(ns,seed,counter,condition,purpose)).digest()[:8],"big")
...
def fy(values,ns,seed,condition="",purpose=""):
    ...
    return a
def token(ns,seed,index,condition="",purpose=""): return hashlib.sha256(stream(ns,seed,index,condition,purpose)).hexdigest()[:16]
def validate_obs(o):
...
def validate_rule(r):
    ...
    for a in atoms:
        ...
        seen.add(key)
    bound={x for a in atoms for x in a["args"]}
    return ("target" in bound) and (r["consequence"]!="NONENABLES(contrast,target)" or "contrast" in bound)
def legal_candidates(max_k=2):
    templates=[]
    for p,n in PRED_ARITY.items(): templates.append([{"predicate":p,"args":list(args)} for args in product(ROLES,repeat=n)])
    atoms=[x for group in templates for x in group]; seen={}
    for k in range(1,max_k+1):
        for tup in product(atoms,repeat=k):
            if len({cj(a) for a in tup})!=k: continue
            for c in CONSEQUENCES:
                r=canonical({"atoms":list(tup),"consequence":c})
...
def compile_facts(history):
    ...
    for i,o in enumerate(history):
        ...
        if i and o["last_action"] is not None:
    ...
    grouped={}
    for f in facts:
        key=cj({"p":f["p"],"a":f["a"]}); grouped.setdefault(key,{"p":f["p"],"a":f["a"],"at":[]})["at"]+=f["at"]
    for f in grouped.values(): f["at"]=sorted(set(f["at"]))
    return [grouped[k] for k in sorted(grouped)]
def grounded_atoms(rule,binding,facts):
    return all(any(f["p"]==a["predicate"] and f["a"]==[binding[x] for x in a["args"]] for f in facts) for a in rule["atoms"])
def acquire(a_histories,pre=None):
    pre_ids={cj(c) for c in (pre or [])}; records={}
    for seed,h in a_histories:
        ...
        for r in legal_candidates(2):
            ...
            for vals in permutations(entities,len(needed)):
                ...
                if grounded_atoms(r,b,fs) and cj(r) not in pre_ids:
                    key=cj(r); rec=records.setdefault(key,{"rule":r,"support_seeds":[],"provenance":[]}); rec["support_seeds"].append(seed); rec["provenance"].append({"seed":seed,"binding":b,"facts_hash":digest(fs)}); break
    out=[]
    for v in records.values():
        seeds=sorted(set(v["support_seeds"]))
        if len(seeds)>=2: v["support_seeds"]=seeds; v["rule_hash"]=digest(v["rule"]); out.append(v)
    return sorted(out,key=lambda x:(cj(x["rule"]),x["rule_hash"]))
def retrieve(k,history):
    fs=compile_facts(history); entities=sorted({x for o in history for x in o["available_actions"]}|{o["last_action"] for o in history if o["last_action"] is not None}); matches=[]
    for rec in k:
        r=rec["rule"]; roles=sorted({x for a in r["atoms"] for x in a["args"]},key=ROLES.index)
    ...
    if conflicts:return {"status":"CONFLICT","candidates":[m[3]["rule_hash"] for m in [top,*conflicts]]}
...
def run(f):
    ...
    ret=retrieve([{ "rule":c,"rule_hash":digest(c)}],f["history"]); pred=c["consequence"] if ret["status"]=="RETRIEVED" else None
    return {"input_hash":digest(f),"facts":fs,"candidate":c,"candidate_valid":True,"retrieval":ret,"prediction":{"consequence":pred,"valid":pred in CONSEQUENCES if pred else False},"cost":cost(f["attempts"],f["terminal"]),"attribution":attribution(f["attribution"]),"novelty":novelty(f["novelty_a"],f["novelty_b"]),"controls":controls(f["history"]),"tokens":{"action":token("LABEL_PERMUTATION",f["seed"],0,purpose="action_token_pool"),"location":token("LABEL_PERMUTATION",f["seed"],0,purpose="location_token_pool")},"rng":[word("TASK_GENERATION",f["seed"],i,purpose="family_b_dependency_subset") for i in range(4)]}
def main():
