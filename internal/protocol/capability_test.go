...
func testContract(expires, notBefore string) provider.ActionContract {
	return provider.ActionContract{
		...
		ExpectedEffect: provider.ExpectedEffect{Resource: "users", ID: "1842", Fields: map[string]any{"active": false, "version": int64(43)}},
		...
		PolicyReference: provider.PolicyReference{PolicyID: "policy-1", Version: "1", Hash: "deadbeef"},
...
func TestMintCapabilityUsesSplitBrainContractHashOnly(t *testing.T) {
	...
	env, cap, err := MintCapability(contract, "exec-1", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
	...
	if err := json.Unmarshal(env.Payload, &fields); err != nil { t.Fatal(err) }
...
func TestCapabilityCannotWidenContractExpiry(t *testing.T) {
	...
	env, cap, err := MintCapability(contract, "exec-2", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
	...
	if err := json.Unmarshal(env.Payload, &payload); err != nil { t.Fatal(err) }
	payload["expires_at"] = now.Add(10 * time.Minute).Format(time.RFC3339Nano)
	forged := mustCapabilityEnvelope(t, payload, env.SignerID, priv)
	if _, err := VerifyCapability(forged, pub, contract, "exec-2", "broker-1", now); !errors.Is(err, ErrCapabilityExpiryWidened) && !errors.Is(err, ErrCapabilityContractHashMismatch) {
		t.Fatalf("forged widened expiry accepted or wrong error: %v", err)
	}
}

func TestCapabilityCannotOutliveShortContract(t *testing.T) {
	now := time.Date(2026, 9, 5, 10, 0, 0, 0, time.UTC)
	contract := testContract(now.Add(90*time.Second).Format(time.RFC3339Nano), now.Format(time.RFC3339Nano))
	pub, priv, _ := ed25519.GenerateKey(nil)
	env, cap, err := MintCapability(contract, "exec-3", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
...
func TestCapabilityCannotBindToDifferentExecution(t *testing.T) {
	now := time.Date(2026, 9, 5, 10, 0, 0, 0, time.UTC)
	contract := testContract(now.Add(time.Minute).Format(time.RFC3339Nano), now.Format(time.RFC3339Nano))
	pub, priv, _ := ed25519.GenerateKey(nil)
	env, _, err := MintCapability(contract, "exec-a", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
...
func TestCapabilityRequiresSingleUse(t *testing.T) {
	...
	contract := testContract(now.Add(time.Minute).Format(time.RFC3339Nano), now.Format(time.RFC3339Nano))
	pub, priv, _ := ed25519.GenerateKey(nil)
	env, _, err := MintCapability(contract, "exec-4", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
	var payload map[string]any
	if err := json.Unmarshal(env.Payload, &payload); err != nil { t.Fatal(err) }
	payload["single_use"] = false
	forged := mustCapabilityEnvelope(t, payload, env.SignerID, priv)
	if _, err := VerifyCapability(forged, pub, contract, "exec-4", "broker-1", now); !errors.Is(err, ErrCapabilityReplayable) { t.Fatalf("replayable capability accepted: %v", err) }
}

func TestCapabilityHonorsContractNotBefore(t *testing.T) {
	now := time.Date(2026, 9, 5, 10, 0, 0, 0, time.UTC)
	nbf := now.Add(2 * time.Minute)
	contract := testContract(now.Add(10*time.Minute).Format(time.RFC3339Nano), nbf.Format(time.RFC3339Nano))
	pub, priv, _ := ed25519.GenerateKey(nil)
	env, cap, err := MintCapability(contract, "exec-5", "broker-1", "postgresql", "boundary-1", now, priv)
	if err != nil { t.Fatal(err) }
...
func mustCapabilityEnvelope(t *testing.T, payload map[string]any, signer string, priv ed25519.PrivateKey) Envelope {
	t.Helper()
	env, err := signPayload("Capability", payload, signer, priv)
	if err != nil { t.Fatal(err) }
	return env
}
