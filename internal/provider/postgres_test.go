...
func TestVerifyExpectedEffect(t *testing.T) {
	...
	result := MutationResult{RowsAffected: 1, ReturnedData: []map[string]any{{"id": int64(1842), "version": int64(43), "active": false}}}
...
func TestVerifyExpectedEffectRejectsMismatch(t *testing.T) {
	...
	result := MutationResult{RowsAffected: 1, ReturnedData: []map[string]any{{"id": int64(1842), "version": int64(44), "active": false}}}
	if err := verifyExpectedEffect(contract, result); err == nil {
...
func TestContractCannotWidenHandlerScope(t *testing.T) {
	...
	contractMax := int64(2)
	if contractMax <= h.MaxScope() {
		t.Fatal("test setup invalid: contract max should exceed handler maximum")
	}
}
