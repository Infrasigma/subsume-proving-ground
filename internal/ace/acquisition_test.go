...
func TestAutonomousAcquisitionPersistsAndTransfers(t *testing.T) {
    ...
    got,err:=ExecuteTask(reg,Task{ID:"novel-surface",Goal:"destination=source+1",Requirements:[]string{"source"},Novel:true,Budget:task.Budget});if err!=nil||got.Status!="verified"||!got.Independent{t.Fatalf("structural transfer failed: %+v %v",got,err)}
    ...
    again,err:=ExecuteTask(reopened,Task{ID:"restart",Goal:"destination=source+1",Requirements:[]string{"source"},Novel:true,Budget:task.Budget});if err!=nil||again.Status!="verified"{t.Fatalf("restart execution failed: %+v %v",again,err)}
}
