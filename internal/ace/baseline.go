...
func(NoopCausal)Predict(m CausalModel,s State,a Action)([]Prediction,error){return []Prediction{{ID:Hash([]any{m.ID,s.ID,a.ID}),ActionID:a.ID,Effects:a.ExpectedEffects,Probability:0.5,StateHash:Hash(s)}},nil}
...
func(DeterministicExperimenter)Propose(m CausalModel,hs []Hypothesis,b ResourceVector)([]Experiment,error){if len(hs)==0{return nil,errors.New("no competing hypotheses")};return []Experiment{{ID:Hash([]any{"experiment",hs,b}),HypothesisIDs:[]string{hs[0].ID},Objective:1,Budget:b,Provenance:Prov("experiment-planner","","discriminate",hs)}},nil}
...
func(StructuralSearcher)Retrieve(t Task,ks []KnowledgeObject)([]KnowledgeObject,error){return StructuralRetriever{}.Retrieve(t,ks)}

type ForwardSimulator struct{}
func(ForwardSimulator)Simulate(s State,a Action,b ResourceVector)([]Prediction,error){if e:=CheckResources(ResourceVector{TimeMS:1},b);e!=nil{return nil,e};next:=s;next.Version++;for k,v:=range a.Arguments{if next.Values==nil{next.Values=map[string]string{}};next.Values[k]=v};return []Prediction{{ID:Hash(next),ActionID:a.ID,Effects:a.ExpectedEffects,Probability:1,StateHash:Hash(next)}},nil}

type BestFirstSearcher struct{}
func(BestFirstSearcher)Search(t Task,s State,skills []Skill,sim Simulator,b ResourceVector)(Plan,error){if len(skills)==0{return Plan{},errors.New("no verified skill applicable to task")};for _,sk:=range skills{ok:=true;for _,p:=range sk.Preconditions{if s.Values[p]==""{ok=false;break}};if ok&&sk.Confidence>0{return Plan{ID:Hash([]any{t.ID,sk.ID}),Goal:t.Goal,Steps:sk.Actions,Expected:[]Prediction{},Cost:ResourceVector{TimeMS:1},Verification:sk.Verification,Provenance:Prov("planner",t.ID,"skill",sk)},nil}};return Plan{},errors.New("no skill satisfies preconditions")}
...
func(e *StateExecutor)Execute(a Action)(State,error){if e==nil{return State{},errors.New("nil executor")};before:=e.Current;for k,v:=range a.Arguments{if e.Current.Values==nil{e.Current.Values=map[string]string{}};e.Current.Values[k]=v};e.Current.Version++;e.Current.Provenance=Prov("executor",before.Provenance.ID,"bounded-mutation",a);return e.Current,nil}

type IndependentVerifier struct{}
func(IndependentVerifier)Verify(a Action,p Prediction,before,after State)(VerificationResult,error){expected:=before;expected.Version++;for k,v:=range a.Arguments{if expected.Values==nil{expected.Values=map[string]string{}};expected.Values[k]=v};ok:=Hash(expected)==Hash(after);status:="failed";if ok{status="verified"};return VerificationResult{Status:status,Expected:a.ExpectedEffects,Observed:a.ExpectedEffects,Independent:true,Provenance:Prov("independent-verifier",a.ID,"compare-state",after)},nil}
...
func(DeclarativeBuilder)Build(c ArchitectureCandidate,s CapabilitySpecification)(ModificationProposal,error){p:=ModificationProposal{ID:Hash([]any{c,s}),Capability:s,Candidate:c,Artifact:fmt.Sprintf("mechanism=%s\ninterfaces=%v\n",c.Mechanism,c.Interfaces),Provenance:Prov("mechanism-builder",c.ID,"construct",s)};return p,nil}
...
func(Controller)Choose(t Task,m SelfModel,b ResourceVector)string{if len(t.Requirements)>0{for _,g:=range m.CapabilityGaps{for _,r:=range t.Requirements{if contains(g.DesiredBehaviour,r){return "specify"}}}};if t.Novel{return "experiment"};return "retrieve"}
