...
type Hypothesis struct { ID, Statement string; Confidence float64; EvidenceIDs []string; Counterexamples []string; Provenance Provenance }
...
type KnowledgeObject struct { ID, Statement string; Level KnowledgeLevel; EvidenceIDs []string; Dependencies []string; Scope []string; Confidence float64; Verification string; Provenance Provenance }
...
type Task struct { ID, Goal string; Requirements []string; Context State; Novel bool; Budget ResourceVector; Provenance Provenance }
...
type Controller interface { Choose(Task, SelfModel, ResourceVector) string }
...
// Deterministic baseline mechanisms are deliberately generic: they provide executable control plumbing,
// while neural/causal/search mechanisms remain replaceable hypotheses behind the interfaces above.
type StructuralRetriever struct{}
func (StructuralRetriever) Retrieve(t Task, ks []KnowledgeObject)([]KnowledgeObject,error){ type scored struct{k KnowledgeObject;s int}; var a []scored; for _,k:=range ks {score:=0; text:=k.Statement+" "+fmt.Sprint(k.Scope); for _,r:=range t.Requirements {if contains(text,r){score++}}; if score>0 {a=append(a,scored{k,score})}};sort.SliceStable(a,func(i,j int)bool{return a[i].s>a[j].s});out:=make([]KnowledgeObject,0,len(a));for _,x:=range a{out=append(out,x.k)};return out,nil }
...
type Runtime struct { Store MemoryStore; Source ObservationSource; Causal CausalEngine; Experiment Experimenter; Abstract Abstractor; Retrieve Retriever; Sim Simulator; Search Searcher; Authorize Authorizer; Execute Executor; Verify Verifier; Diagnose Diagnoser; Transfer TransferMapper; ArchSearch MechanismSearcher; Build MechanismBuilder; Sandbox Sandbox; Integrate Integrator; Controller Controller }
func (r *Runtime) Acquire(t Task)(VerificationResult, error){
    if r.Store==nil||r.Controller==nil{return VerificationResult{},errors.New("runtime missing store/controller")}
    self,_:=r.Store.Self(); op:=r.Controller.Choose(t,self,t.Budget)
    switch op {
    case "specify": return r.resolveGap(t,self)
    case "experiment":
        if r.Experiment==nil||r.Causal==nil||r.Source==nil{return VerificationResult{},errors.New("experimental loop unavailable")}
        exps,e:=r.Experiment.Propose(CausalModel{},nil,t.Budget);if e!=nil||len(exps)==0{return VerificationResult{},fmt.Errorf("no experiment: %v",e)};obs,e:=r.Experiment.Execute(exps[0]);if e!=nil{return VerificationResult{},e};x:=obsToExperience(obs);if e=r.Store.Put(x);e!=nil{return VerificationResult{},e};return VerificationResult{Status:"inconclusive",Independent:true,Provenance:Prov("experiment",x.ID,"observation",obs)},nil
    default:
        if r.Retrieve==nil||r.Search==nil||r.Sim==nil||r.Authorize==nil||r.Execute==nil||r.Verify==nil{return VerificationResult{},errors.New("execution loop unavailable")}
        ks,_:=r.Store.Knowledge();rel,_:=r.Retrieve.Retrieve(t,ks);skills:=make([]Skill,0);_ = rel;plan,e:=r.Search.Search(t,t.Context,skills,r.Sim,t.Budget);if e!=nil{return VerificationResult{},e};if len(plan.Steps)==0{return r.resolveGap(t,self)};a:=plan.Steps[0];if e=r.Authorize.Authorize(a,self);e!=nil{return VerificationResult{},e};before:=t.Context;after,e:=r.Execute.Execute(a);if e!=nil{return VerificationResult{},e};pred:=Prediction{ActionID:a.ID,Effects:a.ExpectedEffects};return r.Verify.Verify(a,pred,before,after)
    }
}
func obsToExperience(o Observation)Experience{return Experience{ID:o.ID,Source:"experiment",Sequence:uint64(o.State.Version),Timestamp:time.Now().UTC(),Observational:o.State.Version==0,Raw:mustJSON(o),Provenance:o.Provenance}}
func mustJSON(v any)json.RawMessage{b,_:=json.Marshal(v);return b}
func(r *Runtime)resolveGap(t Task,self SelfModel)(VerificationResult,error){if r.Diagnose==nil||r.ArchSearch==nil||r.Build==nil||r.Sandbox==nil||r.Integrate==nil{return VerificationResult{},errors.New("capability-acquisition infrastructure incomplete")};spec:=CapabilitySpecification{ID:Hash(t),DesiredBehaviour:t.Goal,Inputs:t.Requirements,Outputs:[]string{"verified task outcome"},Invariants:[]string{"no uncontrolled side effects"},AcceptanceTests:[]string{"behavioural","regression"},ResourceLimits:t.Budget,FailureCriteria:[]string{"verification failure"},RegressionConstraints:[]string{"existing capabilities preserved"},Provenance:Prov("capability-gap",t.ID,"specify",t)};cs,e:=r.ArchSearch.SearchMechanisms(spec,t.Budget);if e!=nil||len(cs)==0{return VerificationResult{},fmt.Errorf("architecture search failed: %v",e)};p,e:=r.Build.Build(cs[0],spec);if e!=nil{return VerificationResult{},e};if e=ValidateCandidate(p);e!=nil{return VerificationResult{},e};rr,e:=r.Sandbox.Validate(p);if e!=nil||!rr.Passed{return VerificationResult{Status:"failed",Independent:true,Provenance:p.Provenance},fmt.Errorf("candidate rejected: %v",e)};if e=r.Integrate.Integrate(p,rr);e!=nil{return VerificationResult{},e};self.Version++;self.Capabilities=append(self.Capabilities,Capability{ID:spec.ID,Name:spec.DesiredBehaviour,Version:self.Version,Provenance:spec.Provenance});if e=r.Store.SetSelf(self);e!=nil{return VerificationResult{},e};return VerificationResult{Status:"inconclusive",Independent:true,Provenance:p.Provenance},nil}
