...
func abstractionObservationFromMethod(m AcquisitionMethodArtifact, taskStructure string, verified, heldOut bool, gain float64, cost ResourceVector) AbstractionObservation {
	p, _ := decodeAcquisitionProcedure(m.Artifact)
	return AbstractionObservation{TaskStructure: taskStructure, Procedure: p, Verified: verified, HeldOut: heldOut, TransferScore: boolScore(verified && heldOut), DiscoveryCost: cost, ObservedGain: gain}
}

func streamForAbstractionVerification(names ...string) []ArchitectureCandidate {
	out := make([]ArchitectureCandidate, len(names))
	for i, name := range names {
		out[i] = ArchitectureCandidate{ID: Hash([]any{"runtime-abstraction-probe", name}), Mechanism: name, Resources: ResourceVector{Compute: float64(i + 1), ExperimentBudget: 1}}
	}
	return out
}
...
func (r *AdaptiveAcquisitionRuntime) learnAbstractionFromVerifiedMethod(method AcquisitionMethodArtifact, telemetry AcquisitionTelemetry, futureSpec CapabilitySpecification, hidden []ProgramTestCase) error {
	...
	r.AbstractionHistory = append(r.AbstractionHistory,
		abstractionObservationFromMethod(method, telemetry.TaskID, true, true, gain, cost),
		abstractionObservationFromMethod(method, Hash([]any{futureSpec.Inputs, futureSpec.Outputs, futureSpec.Invariants, futureSpec.Structure}), true, len(hidden) > 0, gain, cost),
	)
	proposal, err := DiscoverReusableAbstraction(r.AbstractionHistory, 2)
	if err != nil { return nil }
	probeInputs := [][]ArchitectureCandidate{
		streamForAbstractionVerification("probe-a", "probe-b", "probe-c"),
		streamForAbstractionVerification("probe-c", "probe-a", "probe-b", "probe-d"),
	}
	cases := make([]AbstractionVerificationCase, 0, len(probeInputs))
	for _, input := range probeInputs {
		want := referenceProcedure(proposal.Procedure, input, &r.Abstractions, map[string]bool{})
		if want == nil { return nil }
		cases = append(cases, AbstractionVerificationCase{Input: input, Expected: mechanismOrder(want)})
	}
	verified, err := VerifyAcquiredAbstraction(proposal, &r.Abstractions, cases)
	if err != nil { return nil }
