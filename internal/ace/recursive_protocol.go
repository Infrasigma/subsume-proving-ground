...
func RunRecursiveCapabilityProtocolV3() (map[string]float64, error) {
	...
	if err != nil { return nil, err }
	if len(futureCandidates) == 0 || futureCandidates[0].Mechanism != "universal:branching" { return nil, errors.New("installed method did not change future acquisition behavior") }
	...
	if err != nil { return nil, err }
	if _, err := ParameterizedMechanismSearch(c3); err == nil { return nil, errors.New("K0 arithmetic search solved future piecewise task") }
	k2Candidates, err := registry.Apply(spec3)
	if err != nil { return nil, err }
	if len(k2Candidates) == 0 || k2Candidates[0].Mechanism != "universal:branching" { return nil, errors.New("installed M1 was not used on future task") }
	var solved bool
	for _, candidate := range k2Candidates { p, e := (UniversalProgramBuilder{}).Build(candidate, spec3); if e == nil && programFitsJSON(p.Artifact, c3) { solved = true; break } }
