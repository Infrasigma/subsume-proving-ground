...
// General examples are evidence, not an implementation hint. The synthesizer
// searches a compositional program space and validates every candidate against
...
func (e UExpr) eval(env map[string]string) (int,bool,error) {
    ...
    case "add","sub","mul":
        if e.Left==nil||e.Right==nil{return 0,false,errors.New("binary expression missing operand")}; a,ok,err:=e.Left.eval(env);if err!=nil||!ok{return 0,ok,err};b,ok,err:=e.Right.eval(env);if err!=nil||!ok{return 0,ok,err};switch e.Kind{case "add":return a+b,true,nil;case "sub":return a-b,true,nil;default:return a*b,true,nil}
    case "lt","eq","and","or":
        if e.Left==nil||e.Right==nil{return 0,false,errors.New("boolean expression missing operand")};a,_,err:=e.Left.eval(env);if err!=nil{return 0,false,err};b,_,err:=e.Right.eval(env);if err!=nil{return 0,false,err};switch e.Kind{case "lt":if a<b{return 1,true,nil};return 0,true,nil;case "eq":if a==b{return 1,true,nil};return 0,true,nil;case "and":if a!=0&&b!=0{return 1,true,nil};return 0,true,nil;default:if a!=0||b!=0{return 1,true,nil};return 0,true,nil}
    case "not":
...
func (p UniversalProgram) Run(input map[string]string) (map[string]string,error) {
    ...
    exec=func(ss []UStmt) error{for _,s:=range ss{steps++;if steps>1000{return errors.New("program step limit exceeded")};switch s.Kind{
    ...
    default:return fmt.Errorf("unknown statement %s",s.Kind)}};return nil}
...
// GeneralCapabilitySpecification deliberately does not parse the task goal.
// The examples are observations used to infer a behavior contract.
...
// Self-generated adversarial tests probe nearby integer values without using
// the implementation to produce expected answers. They are intentionally
// independent from the initial examples.
