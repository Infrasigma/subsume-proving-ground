...
func GeneralCapabilitySpecification(t Task,cases []ProgramTestCase)(CapabilitySpecification,error){if len(cases)<2{return CapabilitySpecification{},errors.New("general capability requires at least two examples")};inputs:=map[string]bool{};outputs:=map[string]bool{};for _,c:=range cases{for k:=range c.Input{inputs[k]=true};for k:=range c.Expected{outputs[k]=true}};in,out:=make([]string,0,len(inputs)),make([]string,0,len(outputs));for k:=range inputs{in=append(in,k)};for k:=range outputs{out=append(out,k)};return CapabilitySpecification{ID:Hash([]any{"general",t.ID,cases}),DesiredBehaviour:t.Goal,Inputs:in,Outputs:out,Invariants:[]string{"all observed examples preserved"},AcceptanceTests:[]string{"observed-examples","held-out-execution"},ResourceLimits:t.Budget,FailureCriteria:[]string{"wrong output","runtime error"},RegressionConstraints:[]string{"existing capabilities preserved"},Provenance:Prov("capability-discovery",t.ID,"infer-from-examples",cases),KnownExamples:cases},nil}

type UniversalMechanismSearch struct{}
func (UniversalMechanismSearch)SearchMechanisms(s CapabilitySpecification,b ResourceVector)([]ArchitectureCandidate,error){strategies:=[]string{"straight-line","branching","compositional-iteration"};out:=make([]ArchitectureCandidate,0,len(strategies));for _,strategy:=range strategies{out=append(out,ArchitectureCandidate{ID:Hash([]any{s.ID,strategy}),Mechanism:"universal:"+strategy,Interfaces:[]string{"typed-key-value-input","executable-program"},Advantage:"compositional synthesis",Assumptions:"bounded integer/boolean primitives",Resources:b,Tests:s.AcceptanceTests,RegressionRisks:s.RegressionConstraints,Provenance:Prov("architecture-search",s.ID,"strategy",strategy)})};return out,nil}

type UniversalProgramBuilder struct{}
func (UniversalProgramBuilder)Build(c ArchitectureCandidate,s CapabilitySpecification)(ModificationProposal,error){if len(s.KnownExamples)<2||len(s.Inputs)==0||len(s.Outputs)==0{return ModificationProposal{},errors.New("insufficient behavioral evidence")};vars:=append([]string{},s.Inputs...);out:=s.Outputs[0];var exprs []UExpr;for _,v:=range vars{exprs=append(exprs,UExpr{Kind:"var",Value:v})};for n:=-2;n<=2;n++{exprs=append(exprs,UExpr{Kind:"const",Value:strconv.Itoa(n)})};base:=append([]UExpr{},exprs...);for _,a:=range base{for _,b:=range base{aa,bb:=a,b;exprs=append(exprs,UExpr{Kind:"add",Left:&aa,Right:&bb},UExpr{Kind:"sub",Left:&aa,Right:&bb},UExpr{Kind:"mul",Left:&aa,Right:&bb})}}
    if strings.HasPrefix(c.Mechanism,"universal:branching")||strings.HasPrefix(c.Mechanism,"universal:compositional"){for _,v:=range vars{for _,e:=range exprs{for _,cmp:=range []string{"lt","eq"}{for _,n:=range []int{-1,0,1}{cc:=UExpr{Kind:cmp,Left:&UExpr{Kind:"var",Value:v},Right:&UExpr{Kind:"const",Value:strconv.Itoa(n)}};ee:=e;p:=UniversalProgram{Statements:[]UStmt{{Kind:"if",Cond:&cc,Then:[]UStmt{{Kind:"assign",Target:out,Expr:&ee}},Else:[]UStmt{{Kind:"assign",Target:out,Expr:&ee}}}}};if programFits(p,s.KnownExamples){return encodeUniversal(p,s,c)}}}}}}
    for _,e:=range exprs{ee:=e;p:=UniversalProgram{Statements:[]UStmt{{Kind:"assign",Target:out,Expr:&ee}}};if programFits(p,s.KnownExamples){return encodeUniversal(p,s,c)}};return ModificationProposal{},errors.New("universal synthesis exhausted search space")}
