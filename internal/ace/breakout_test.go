...
func TestUniversalSynthesisEscapesAffineCeiling(t *testing.T) {
    ...
    var prog UniversalProgram
    if err:=json.Unmarshal([]byte(chosen.Artifact),&prog);err!=nil{t.Fatal(err)}
...
func TestUniversalProgramSupportsCompositionAndBranching(t *testing.T) {
    x:=UExpr{Kind:"var",Value:"x"};zero:=UExpr{Kind:"const",Value:"0"};one:=UExpr{Kind:"const",Value:"1"}
    cond:=UExpr{Kind:"lt",Left:&x,Right:&zero}
    neg:=UExpr{Kind:"sub",Left:&zero,Right:&x}
    plus:=UExpr{Kind:"add",Left:&x,Right:&one}
    p:=UniversalProgram{Statements:[]UStmt{{Kind:"if",Cond:&cond,Then:[]UStmt{{Kind:"assign",Target:"y",Expr:&neg}},Else:[]UStmt{{Kind:"assign",Target:"y",Expr:&plus}}}}}
    got,err:=p.Run(map[string]string{"x":"-2"});if err!=nil||got["y"]!="2"{t.Fatalf("branch failed: %v %v",got,err)}
    got,err=p.Run(map[string]string{"x":"2"});if err!=nil||got["y"]!="3"{t.Fatalf("composition branch failed: %v %v",got,err)}
