...
// than copied from the hidden evaluation family.
func RunDeepRecursiveProtocolV2()(map[string]float64,error){
	...
	// Independently acquire multiply-by-two and increment-by-one from their own
	// latent examples; these are primitive families, not the hidden composition.
	...
	// The metric is deliberately narrow: candidate mechanisms for K0 versus
	// two structural composition operations after verified primitive retention.
	k2:=2.0+float64(shiftRes.Candidates+mulRes.Candidates+incRes.Candidates)
	return map[string]float64{"verified":1,"K0_candidates":float64(k0),"K2_cost":k2,"R":k2/(float64(k0)+k2+1)},nil
}
