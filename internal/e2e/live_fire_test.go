...
func runSandboxAgent(t *testing.T, binary, socket string, raw []byte, args ...string) []byte {
	...
	if len(agentArgs) == 0 { agentArgs = []string{"/run/aacr/broker.sock"} }
...
func TestM6LiveFire(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	liveVerifier := liveVerifier{publicKey: agentPublic}
	liveProvider := liveProvider{postgres: postgres}
	liveEvidence := liveEvidence{pool: pg.pool}
	b, err := broker.New(liveVerifier, l, liveProvider, liveEvidence, "broker-live", "postgresql", "bubblewrap-live", brokerPrivate)
	if err != nil { t.Fatal(err) }
